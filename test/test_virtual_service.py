...
def test_virtual_service_basic():
    ...
    actual_results = generate_config.generate_virtual_service(
        config, "istio-egressgateway.istio-system.svc.cluster.local"
    )
    assert expected_results == actual_results


def test_virtual_service_complex():
    config = get_yaml("complex.yaml")
    expected_results = get_yaml("results_check/virtual_servicecomplex.yaml")
    actual_results = generate_config.generate_virtual_service(
        config, "istio-egressgateway.istio-system.svc.cluster.local"
    )
    assert expected_results == actual_results
